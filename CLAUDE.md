...
## 2026-09-25 — Hata yakalama, tema yönetimi ve PostgREST filtre cila
- **Canlı tema dinleme (2.5 & 2.7):** `ThemeProvider.tsx`'te `theme === 'system'` iken `window.matchMedia('(prefers-color-scheme: dark)')` dinleyicisi `addEventListener('change', ...)` ile bağlandı ve cleanup'ta kaldırıldı. `toggleTheme` `light → dark → system → light` 3 durumlu döngüye çevrildi (Ayarlar'daki 3 butonlu seçiciyle çakışmaz).
- **Açılış beyaz flaşı ve tema rengi (2.4):** `electron/main.ts`'teki `BrowserWindow` `backgroundColor`'ı `nativeTheme.shouldUseDarkColors` API'sine bağlandı (koyu: `#171a1a`, açık: `#f5f7f8`). Node.js ana sürecinin renderer `localStorage`'ına doğrudan/senkron erişememesi nedeniyle sistem teması referans alındığı kod içinde detaylı açıklandı. Kullanıcı sistem temasından bağımsız bir tercih yaptıysa `public/theme-init.js` DOM parse edilir edilmez `dataset.theme`'i düzeltir.
  🔴→✅ **Bu turun ilk hali `index.html`'e INLINE `<script>` yazmıştı — üretim CSP'sini (`script-src 'self'`, nonce/`unsafe-inline` yok, `vite.config.ts`) ihlal ediyordu.** Paketlenmiş uygulamada tarayıcı bu scripti sessizce engelleyecekti (flaş önleme hiç çalışmaz + konsola CSP hatası düşer); `npm run build` bunu yakalamaz, yalnızca gerçek bir tarayıcıda/CSP ile çalıştırınca görülür. `dist/index.html`'i başsız Edge'de `file://` ile açıp doğrulandı. Düzeltme: script `public/theme-init.js`'e taşındı, `<script src="./theme-init.js">` ile yüklendi (aynı-köken dosya yükü `'self'` altında serbest). Tekrar doğrulandı: `dist/index.html` başsız Edge'de `data-theme="light"` + doğru `background-color` DOM'a yansıyor, konsolda CSP ihlali yok.
- **İşlem geçmişi araması kaçış karakterleri (2.3):** `auditLogService.ts` içinde `filter.query` PostgREST `or()` filtresine gönderilmeden önce `\`, `%`, `_` ve `,` karakterleri kaçırıldı (`lib/queryEscape.ts`). Saf fonksiyon olarak ayrı tutulduğu için Tuzak #11'e takılmayan 6 yeni birim testi eklendi (`queryEscape.test.ts`).
- **React Error Boundary (2.2):** `src/components/ErrorBoundary.tsx` ve `.css` yazıldı; `src/main.tsx`'te en dış katmana sarıldı. Render hatalarında beyaz ekran yerine "Bir şeyler ters gitti" başlığı, hata mesajı/stack dökümü, "Hata Detayını Kopyala" (panoya kopyalama ve durum bildirimi) ile "Yeniden Dene" / "Sayfayı Yenile" butonları sunuldu. CSS'te sabit hex kullanılmadı, `--page`, `--surface`, `--ink`, `--muted`, `--danger` token'ları ile koyu ve açık temaya tam uyumlu (Tuzak #10).
